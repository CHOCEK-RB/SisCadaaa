<script lang="ts">
  ...
  import { courseExtractors } from "$lib/config/access_courses";
  ...
  const currentCourses = $derived.by(() => {
    ...
    const dataWrapper =
      role === "teacher" ? { groups: rawData } : { courses: rawData };
...
<div>
  {#if currentCourses.length > 0}
    <div class="space-y-2">
      {#each currentCourses as course, i (course)}
        <a href={course.link} class="no-underline">
          <div
