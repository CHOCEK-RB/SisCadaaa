<script lang="ts">
  ...
  const coursesByPeriod = data.courses || {};
  const error = data.error;
</script>

<svelte:head>
  <title>Dashboard - Sisacad</title>
</svelte:head>
...
<div class="flex-1 space-y-4 bg-background p-0">
      ...
      >
        ...
        >
          <h1 class="text-2xl leading-tight font-bold sm:text-3xl">
            ¡Hola de nuevo, {userProfile?.firstName}!
          </h1>
  ...
  >
    {#if error}
      <div
        class="mb-6 flex items-center rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-800 dark:border-red-700 dark:bg-red-900/20 dark:text-red-300"
        role="alert"
      >
