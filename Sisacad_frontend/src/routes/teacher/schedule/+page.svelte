  ...
  {:else if allScheduleGroups && allScheduleGroups.length > 0}
    ...
    >
      ...
      <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
        <!-- Esta es la línea (course.id) que causaba el error -->
        {#each coursesSummary() as { course, groups } (course.id)}
          <div class="rounded-lg border border-gray-200 bg-gray-50 p-4">
            ...
            <div class="space-y-1">
              <!-- (group.id) aquí está bien porque el backend ya los desduplicó -->
  ...
  {:else}
    <div class="rounded-lg border bg-white p-6 text-center shadow-md">
      ...
      <p class="mt-2 text-sm text-gray-500">
        Los horarios de tus grupos asignados aparecerán aquí.
      </p>
    </div>
  {/if}
</div>
